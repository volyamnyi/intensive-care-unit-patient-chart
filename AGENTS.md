...
**2026-09-18: Drug-interaction warnings in the prescription sheet — issue #304 CLOSED, CI run `35609683523` ALL 6 JOBS GREEN (E2E 420 passed / 0 failed)** — server-side, non-blocking "planned-overlap" interaction warnings for a `PrescriptionList`. P1 (DB+entities): `med/005-drug-interactions.sql` adds `drug_interaction_drugs` (ATC-keyed) + `drug_interaction_pairs` (canonical a<b ATC pair, severity, `rowHash`) + `prescription_items.medicine_atc_code` (7-char, nullable); entities/repos in the `medicationsheet` namespace. P2 (import): `DrugInteractionImportService` full-sync (single med tx: parse+validate → wipe+insert, roll back on any failure; strict `^[A-Z][0-9]{2}[A-Z]{2}[0-9]{2}$` ATC, severities low/medium/high/critical, dedup key a|b|severity|sha256, confidence-wins per ATC, unknown-ATC references drop) → `POST /api/admin/drug-interactions/import` (multipart ≤20MB, `hasRole('ADMINISTRATOR')`, audited; bad JSON→400, 0 valid rows→422 `BUSINESS_RULE`; writes `system_settings` `drug_interactions.last_import_at`). Admin UI tab «База взаємодій» (`AdminPage.tsx`: file upload, report grid drugs/interactions/skipped/duration/sourceHash, last-import timestamp). P3 (warning service): `DrugInteractionWarningService.computeWarnings` — planned period = `[min,max dayDate]` over parts `isPlanned && !isPlannedFinished && !isCompleted`, overlap inclusive; dangerous (medium/high/critical) pairs with ATC∩planned → bidirectional `PairWarning` (+ `MissingAtcNotice` for planned items lacking ATC); `GET /api/prescriptions/{listId}/interactions` (`PATIENT_VIEW`); `PrescriptionItemService.addItem` persists ATC (`normalizeAtc`, blank→null). `PrescriptionInteractionsResponse`/`DrugInteractionImportReport` DTOs (wire field `interactions`, NOT `pairs`; `LocalDate`→ISO string). P4 (frontend): `types/medication.ts` (severity `'medium'|'high'|'critical'`), `api/medication.ts` (`getInteractions` + `drugInteractionAdminApi.importDataset`), `MedicineSearchInput` threads `itemKindAtc`→add, `PrescriptionGrid`→`PrescriptionSpreadsheet`, `index.css` `interactionBlink` + `.interaction-warn` + `prefers-reduced-motion` gate. UI: warned item name blinks (`.interaction-warn` `<p>`), planned overlap cells `2px solid #EF4444` border + `data-interaction-warn="true"` (status bg wins), cell tooltip lists severity-sorted pairs (`SEVERITY_ORDER{critical:3,high:2,medium:1}` + `SEVERITY_LABELS{критично/високо/помірно}`); `PrescriptionDetailPage` fetches (non-blocking catch) + refreshes after every mutation + missing-ATC `Alert warning`. Local evidence: `DrugInteractionWarningServiceLogicTest` 5/5, `DrugInteractionImportServiceParseTest` 8/8, `PrescriptionDetailPage.test.tsx` 16/16 (incl. 4 #304 cases; use relative `mockGetInteractions` baseline — the mock returns a NEW object each call so strict `toHaveBeenCalledTimes` is flaky), Vitest 930/100 green, lint 0 errors, tsc build clean, mis-stub `fixtures.json` gains `itemKindATC` on 5 meds (Paracetamol=N02BE01, Ibuprofen=M01AE01, Aspirin=B01AC06, Dobutamine=C01CA02, Glucose5%=B05BA01; `MisServiceImpl.parseMedicineList` reads `itemKindATC`/`itemKindAtc` L181; `stub-contract-test.cjs` 13/13). E2E `tests/specs/doctor/prescription-interactions.spec.ts` (doctor project, serial): admin-2-drug import (N02BE01↔M01AE01 matching stub ATC), RBAC (doctor 403 import / admin ok / nurse read+403 import), reads add+plan-then-warn via `GET .../interactions` (partner name = dataset `ukrainian_raw`), invalid payloads (400/422), UI name-blink + `td[data-interaction-warn="true"]`; names `IT304-*` so the concurrent doctor specs (`ensureMedicationItem('Paracetamol 500 mg')` on the same shared stub list) can't collide; afterAll deletes the added items. Counts synced (README + AGENTS): backend 390 main/185 test (med 93/30), frontend 930 vitest/100 files/146 sources, E2E 98 specs/451 tests; README + AGENTS API tables gain both endpoints. Live E2E `npx playwright test --project=doctor-chromium prescription-interactions.spec.ts` = **20/20 green** (15 setup + 5 spec; local stack: stub MIS 9099 + backend vs 4 local DBs + Vite 5173; `APP_LDAP_ENABLED=true` + `APP_TEST_USERROLE1..9` set so `UserSeedService` seeds real roles — first local boot had left users 2/4/5/6 as GUEST, blocking the AD-identity setups' admin promote path). Two real bugs caught live + fixed: (1) `DrugInteractionImportService` re-`save()`d a FRESH `SystemSettings` on every import → 2nd import violated `system_settings_key_key` duplicate → 500 (rolled back, dataset untouched) → now mutates the existing row in place (`findByKey(...).orElseGet(...)`, same pattern as `ProductionNormativeService.write`); (2) `DrugInteractionWarningService.computeWarnings` conflated "list has no items yet" (fresh list) with "list missing" → fresh-list `GET .../interactions` 404'd → now `listRepository.findById` gates the 404 and an item-less list returns an empty 200 response. Third fix (spec-only): `warnings[].nameUk` is the item's stored name, the dataset `ukrainian_raw` is the PARTNER's display name (`otherNameUk`) — test assertion corrected. Final green loop: backend `mvn test -pl medication-sheet -am` all green (common 25 + med 25 classes, 0 failures, incl. `DrugInteractionWarningServiceLogicTest` 5/5 + `DrugInteractionImportServiceParseTest` 8/8 + `PrescriptionControllerTest` 44/44), frontend lint 0 errors / tsc clean / Vitest 930/100. **Accepted deltas vs the plan (documented in the issue close comment, no code change):** (1) §7 hover renders as per-cell dose-tooltip aggregate (`⚠ {partner} ({severity-label}) — {text}`, severity-sorted) instead of a name-hover conflict-group highlight + «небезпечний період: dd.MM–dd.MM» line; (2) §5 alert wording is «Немає ATC-коду: {names}…» (no explicit «Відділ ІТ» phrasing); (3) overlap cells = red border only (plan's `#FECACA` fill under status bg not added); (4) import matrix covered by unit + E2E (no separate backend test-dataset resource / JUnit integration class — small dataset is inline in the spec); (5) `computeWarnings` aggregation paths (low-exclusion, symmetry-once) E2E-only; `PairWarning.severity` = first matched row's severity when a pair has several (texts joined « / »).

**2026-09-17: Fix-all-fails round — foreign CORS WIP adopted, legacy SecurityConfig shadow deleted, CRLF seed flake fixed, #303 E2E flake fixed; CI run `35248814781` ALL 6 JOBS GREEN (E2E 411 passed / 0 failed)** — owner decision: foreign WIP kept (`setAllowedOriginPatterns(["*"])` stays, `application.yml` ldap-flip reverted to `false`), fix everything incl. #303. `187df2f`: WIP adopted (dead `allowedOrigins` @Value field removed) + `CorsConfigTest` rewritten to the patterns contract (was 2 failures). `28e7c63`: `TpLl02SeedValidationTest` CRLF-normalizes the seed read — root cause: file is LF in git, Windows checkouts materialize CRLF (`core.autocrlf`), breaking the exact multi-line assertion (Linux CI held LF); seed file untouched (Liquibase checksums). **Root-cause hunt of the day**: integration 9+1 failures were NOT the permission matrix — the runtime URL ceiling `[HEAD_OF_DEPARTMENT, DOCTOR]` proved (via surefire authorization trace) a **legacy duplicate `com.superhumans.config.SecurityConfig` shadowing the real one** (foreign untracked file in `icu-chart/.../config/`, pre-dates dynamic RBAC, AGENTS.md already warned it must stay out of compiles); deleted from disk (untracked, nothing to commit) + `mvn clean` to purge the orphan `.class` (incremental compile never deletes it) → 8/9 healed, full AdminPermissions class green on clean DBs, then FULL integration profile 233/233 green. Remaining Order(5) 401-anomaly dissected to a test-harness artifact (verbatim replay passes; live node replay of the exact flow returns 200) — resolved once the shadow was gone. `e912877` (#303): failure screenshot showed a stray «fail process» modal swallowing clicks — helper now dismisses stray dialogs via Escape + verifies `elementFromPoint` lands in the row (re-center retry, fail-fast names the covering element). CI `35248814781` (head `e912877`): 6/6 green, E2E 411/0/4 in 22m6s → #303 closed. Lessons: (1) bare `-pl` without `-am` resolves stale `~/.m2` jars (bit twice: DeptServiceTest compile + phantom solo-401 — always `-am`); (2) after DELETING a source, `mvn clean` (orphan classes shadow); (3) `node --test <dir>` doesn't discover here — explicit file; (4) `-Dsurefire.failIfNoSpecifiedTests=false`; (5) surefire `-output`/`TEST-*.xml` carry full server TRACE logs — read them before theorizing; (6) `Set-Cookie`/audit DEBUG+TRACE line pairs double-count grep hits.

**2026-09-17: Test isolation from real MIS/LDAP in CI — series #297–#302 code-complete; CI run `35224795890` proves the criterion (415 E2E executed with zero MIS/LDAP secrets, 410 passed / 1 pre-existing flake / 4 intentional skips)** — #297 (`163e84e`): MIS sidecar-stub (`tests/mis-stub/`: `node:http`-only `/token`+`/api/run`+`/doc/*`+`/health`, hand-crafted PII-free `fixtures.json` mirroring seed patients 900001/900002 + anchors 13373/10101/10102/10401-MOV/10501-dept2, `stub-contract-test.cjs` 13/13 invisible to Playwright by filename). #298 (`ab5ec5d`, +12 unit tests, no `src/main` changes): `LdapDisabledInTestTest` (ungated — no directory beans + LOCAL login + JWT round-trip without LDAP), `NoRealMisInTestsTripwireTest`, `MisStubContractTest` (fixtures through `MisServiceImpl` parsers). #299 (`11dbc67`, workflow only): `APP_LDAP_ENABLED=false` in 3 jobs, new `Start MIS stub` step, all 6 `APP_MIS_API_*` secret mappings removed (stub `127.0.0.1:9099` + `e2e-stub` creds). #300 (`4dc7fe7`, 20 E2E files): fixed stub patient 10102 + `Paracetamol 500 mg`, candidates/docs skips → strict expects, `real MIS` grep clean. #301 (`e476de4`+`a808c1a`): Test-isolation matrix docs; LDAP-default drift corrected (code default is `false`; #302 caught a foreign uncommitted WIP flipping it to `true`, neutralized by the explicit CI flag). Local evidence: stub 13/13, unit 1069 run (3 pre-existing failures: 2× dirty-WIP `CorsConfigTest`, 1× Windows-Cyrillic `TpLl02SeedValidationTest`), integration 233 run (9+1 proven pre-existing via pristine-tree A/B — same 4 classes fail without this series), Vitest 100/926, build green, live backend boot vs stub (Tomcat up, zero `LDAP authentication is ENABLED`, autoconfig report excludes `LdapConfig`/`LdapAuthService`, anon/unknown logins 401). CI `35224795890` (head `a808c1a`): 5/6 jobs green (Code Quality, Backend Tests, Backend Integration, Frontend Tests all green incl. series changes); E2E 410 passed / 1 failed / 4 skipped in 21m17s — the single red is the documented pre-existing `wizard-checkbox-surface` handover point `(0.95,0.05)` (byte-identical signature, file untouched by series → follow-up issue #303, needs owner look); 4 skips are intentional stale-spec markers (tp-ll-02-conditional/validation). Bugs caught live: medicine envelope key is `medicineItemKindDetails` (NOT the procedure name — contract test caught it); `node --test <dir>` fails in this env (use explicit file); PowerShell `mvn -D` args with dots must be quoted; surefire needs `-Dsurefire.failIfNoSpecifiedTests=false` (not `-DfailIfNoTests`); `DocumentUrlAvailability` is fail-open except 404 → stub serves `/doc/*` 200 (unknown ids 404); `candidates` needs seed local orders → stub roster must contain 900001/900002. Series rule proven: zero `src/main` + `frontend/src` diffs across all 6 commits.

**2026-09-14: PPE safety notices in prosthetics wizard — epic #293–#296 code-complete; CI run `34862590047` CANCELLED on 40-min E2E timeout (triage below, NOT this epic)** — non-blocking `Alert`-banners about mandatory PPE on TP-LL-02 steps, single insertion point (`getPpeNotices(step?.id).map(...)` at the top of the `step-fade-in` wrapper in `WizardScreen.tsx`, after the step title, before the form). #293 (`f8819f3`): 5 PNG `docs/images→frontend/public/ppe` (kebab-case) + `warning` variant in `ui/alert.tsx` (shadcn MCP, theme-safe amber) + `src/prosthetics/ppeNotices.ts` (`PPE_NOTICES_BY_STEP`: 6 stepIds / 7 notices; `e0000024` carries `[full-kit, nitrile]`) + 18 config tests. #294 (`ad584a4`): `PpeNoticeBanner` full-kit branch (4 photos, `figure/figcaption`, `data-testid`, zero interactive elements) + insertion. #295 (`72dd01b`): nitrile branch (`default` Alert + `Hand`, 1 photo left/text right). Owner correction applied mid-epic: nitrile entry №1 is Етап 3/КРОК 1 `e0000024` (shared with full-kit), `e0000021` has no notices. #296: `ppe-notices.spec.ts` 4/4 live (walk e22→e26 + e28→e32, stack order, naturalWidth>0 image proof, non-blocking «Готово», 360/768 no-overflow, zero console.error) + `WizardScreenPpeNotices.test.tsx` wiring (2/2). Local evidence: Vitest 100 files/926 green, lint 0 errors, tsc/build green. CI `34862590047` (head `5b8abfa` + pre-existing `726f98a`): 4/6 jobs green (Code Quality, Backend Tests, Backend Integration, Frontend Tests); E2E cancelled by the 40-min job timeout at test ~351/415 — `ppe-notices.spec.ts` 4/4 green in CI with zero retries (15:43–15:45Z). Red-triage (full job log in `ci-triage/job-104039177388-log.txt`, 18 MB): failures are systemic and pre-date/outside this epic — first failure 15:38Z `doctor-day-flow` (ICU, untouched files, flaky→healed on retry), then `setup-order-documents` (`waitForResponse` 15 s timeout, page fetch never fires — file touched by `726f98a`, needs owner look), then a cascade across `setup-patient-list`/`brak-audit-pdf`/`brak-branch`×9/`cross-feature`×3/`failure` (pure-API tests dying on hanging `GET /instances` >30 s — backend↔MIS stall, no backend code changed in this push; 60 s expect-timeout × retries burned the budget). Re-run `34947266228` (head `be0f785`, +docs only): 394 passed / 2 failed / 19 skipped in 24 min — MIS window passed, `ppe-notices` 4/4 green again with zero retries. The 2 remaining reds are both pre-existing/outside this epic: (1) `setup-patient-list:36` strict-mode violation (`getByRole('cell')` matches 2 cells — regressed by `726f98a`'s fixture-caching refactor, needs owner look); (2) `wizard-checkbox-surface` handover point `(0.95,0.05)` — byte-identical failure with the PPE insertion neutered (`{null && ...}`), fails the same way in CI, so unrelated to this epic (likely latent overlap/focus issue on the TP-UL-01 handover row, was green in older runs — needs owner look). Bugs caught live: blanket `*.png` gitignore hid even the 4 pre-existing `/ppe` images from git → `!frontend/public/ppe/*.png` exception (+ all 9 tracked); `backend/common` boot has NO feature modules (all prosthetics API → 404→401 on `/error`, proven via TRACE — always boot the full `app-*.jar` for E2E); PowerShell `Get-Content|Set-Content` round-trip corrupts UTF-8 (never use it on i18n files — use `write`/`edit`); U+2019 vs U+0027 apostrophe mismatch breaks getByText (seed uses U+0027); walk specs need `test.setTimeout` (default 30s too short). Pre-existing, untouched: `wizard-checkbox-surface` walk fails at handover (0.95,0.05) point — proven NOT this epic via A/B with insertion neutered (`{null && ...}` → same failure); AD-identities setup fails locally (needs corporate AD); users 2/4/5/6 are GUEST since 2026-09-11 (pre-existing); foreign untracked `icu-chart/.../config/SecurityConfig.java` (legacy duplicate FQN — must stay out of full-boot compiles). Seed-users table below rewritten from owner data (env names only); local runs need `APP_LDAP_ENABLED=true`.

**2026-09-13: Legacy import MedicineList→Medication Sheet — implementation + dry-run proven (issues #285–#292: #285, #286, #287, #288 (sign-off), #289, #290, #291 CLOSED; #292 this docs entry; code UNCOMMITTED, CI not run)** — one-shot `migration`-profile importer (`backend/medication-sheet/.../migration/`: runner two-pass P-then-V, per-list med tx, `ImportConverters` pure rules, streaming `;`-CSV reader, tolerant Jackson shredder of all 3 cell generations, `VitalMergeService` fill-empty-only, list-driven `ImportRollbackService`, 14-check `ImportValidationService` with non-zero exit) + staging DDL `med/003` (`import_run/map/quarantine`, `UNIQUE(old_kind,old_id)` idempotency guard). Dry-run on scratch `*_t` DBs: 1555 lists (1544 P + 11 shells `(імпорт ЛЖП №…)`) / 10945 items / 264304 days / 1057216 parts; vitals 16196+8251+1314=25761; quarantine 289/1482/6; orphans 0; audit 2372 IMPORT rows; re-run 0/2372; rollback twice to zeros; validate 14/14. Bugs caught live: stale fat JAR (always `clean package` — nested modules silently stale otherwise), transient FK refs (use `getReferenceById`), CSV bare-quote + chunk-boundary splits, non-unique legacy element/day IDs (344/3297 cross-list dups → composite map keys; `save()`+assigned composite key = silent `merge()` overwrite!), single-pass spurious shells (201 vs 11). Known limits: executions flags-only (no `prescription_executions`), no signatures, future dates as-is, `createdBy=0`+IMPORT audit, MakeDEDocument ignored, MIS pre-check skipped locally (mandatory in prod). MIS untouched (read-only `getPatient` only).

**2026-09-11: Prescription PDF Form №003-4/о — implementation (issue #284, code-complete, CI GREEN run `34645058937` all 6 jobs green on first attempt)** — batch PDF for a `PrescriptionList`: new `medicationsheet.pdf` package (`PrescriptionPdfService` orchestration + stateless ZIP, `PrescriptionPdfPagePlanner` pure deterministic chunks 10 dates × 9 items → `PrescriptionPdfPagePlan` (no iText), `PrescriptionPdfUsernameResolver` (`nameUUID(login)→login` via `UserRepository`, attending = first DOCTOR by id, fail-closed, `secondPersonId` never resolved), `PrescriptionPdfDataLoader` (one read-only snapshot + MIS patient + `institution_name/edrpou`), `PrescriptionPdfRenderer` (iText 8.0.5 A4 landscape, 13 cols / 21 rows, DejaVu IDENTITY_H, fresh fonts per render — singleton reuse throws `Pdf indirect object belongs to other PDF document`, caught live), `findByDayPartIdIn` batch (no N+1); endpoints `GET /{id}/pdf/info|/pdf/file` (STORED ZIP `prescription-{id}.zip`, `X-Total-Pages`, no PII) `|/pdf/pages/{i}` all `PATIENT_VIEW`; controller `@Tag` fixed `003-15/о→003-4/о`; itext-core added to `medication-sheet/pom.xml` (allowlist already permits). Frontend: `prescriptionApi.getPdfInfo/getPdfZip/getPdfPage` + «Завантажити PDF»/«Друкувати PDF» on detail page (ZIP download, per-sheet `printPdfBlob`, nurse-visible). Tests: planner 17-state matrix, signatures (incl. `markCompleted`-without-execution, combined `nurseName`, no raw UUIDs), renderer (`PdfTextExtractor`, whitespace-normalized — extractor splits long lines), controller slice (ZIP/`%PDF`/404/headers), service integration (Docker PG, `em.clear()` before reads — EAGER bags go stale in setup tx), E2E `prescription-pdf.spec.ts` (3/3 live: ZIP entries/`%PDF`/names/no-PII, single sheet, 404 + nurse read). Local evidence: med unit 184/184, integration 4/4, Vitest 883 + 1 pre-existing `ProcessLayout` failure (proven pre-existing via stash — fails without these changes), E2E spec 3/3 live. Pre-existing, untouched: `CorsConfigTest` (foreign dirty file), 3 broken icu-chart integration test files (block local full `install`), `ProcessLayout.test.tsx`. Plan in issue body re-verified against code before implementation (11 codebase corrections).
...
**2026-09-09: PDF transfer to MIS banned, in-module printing (issue #269, CLOSED — CI green run `34390932379`, head `136ce12`; close-summary recorded in issue comments)** — MIS is now strictly read-only with no exceptions: deleted `MisService.sendPdf` + impl (was a stub: audit + log only, never called MIS), stripped `PdfGeneratorService` (local generate/store, single save), deleted `TransferStatus` enum + entity fields + `findByTransferStatus` + `PdfResponse` transfer fields + `GET /pdf/status` (404-regression test), new `GET /clinical-days/{id}/pdf/file` serves bytes (`application/pdf`); Liquibase `icu/007-drop-pdf-transfer.sql` drops the 3 columns (full rollback). Frontend: new `lib/printPdf.ts` (hidden-iframe print) + Vitest; ICU day page gains «Завантажити»/«Друкувати» on closed days; prosthetics Done/Failed screens gain «Друкувати PDF». Tests: `PdfGeneratorServiceTest` local-only (+ getPdfBytes cases), `PdfControllerTest` bytes + status-404, new `MisWriteBanTripwireTest` (no `sendPdf` in `src/main`), endpoints + PatientDayPage Vitest, pdf-generation E2E (+ file-bytes `%PDF` + status-404). Docs: MIS Data Policy absolute in AGENTS.md/README/Javadoc, UseManual print section, runbook/SKILL mock-vars rewritten to single-mode reality. Pre-flight: `mvn test-compile` clean, lint 0 errors, `tsc` clean, build green, `--list` 412 tests.

**2026-09-09: Real-MIS E2E stabilization — dynamic credentials, roster bulk-fetch, triage to green (issue #264 (WireMock removal) + #265–#268 tests, all OPEN; status + red-triage recorded in issue comments)** — E2E migrated off hardcoded users/data onto real MIS: new `tests/helpers/test-users.ts` (`testUser(slot)` reads `APP_TEST_USERNAME1..9`/`APP_TEST_PASSWORD1..9`, throws on missing, slot map 1/2=DOCTOR 3/4=NURSE 5=HOD 6=ADMIN 7/8=PROSTHETIST 9=PROSTHETICS_ADMIN) + new `tests/helpers/medication.ts` (`firstMedicationPatient` via `?module=medication`, always-open `ensureMedicationList`, `ensureMedicationItem` with live-catalog drug, roster/drawer nav + direct-URL setup); 40+ specs migrated (auth.setup USERS, ad-auth seedAdminHeaders, prosthetics-flow logins, object/positional forms, UI fill pairs); 3 medication specs rewritten (add-drug, doctor/nurse day-management) with dynamic patient + live catalog drug; frontend ICU dashboards match search by patient ID too; stale `tests/diag-patient.cjs` (hardcoded creds) deleted. Workflow: `APP_TEST_*` moved to e2e-job-level env from secrets (was step-scoped to Start-backend only → undefined under Playwright), hardcoded values removed, `environment: APP` declared (secrets are environment-scoped; missing env = empty echo + testUser throw, proven in triage). Backend: `MisServiceImpl` + `MisAuthService` replace deleted `WireMockMisServiceImpl` + `mis-wiremock/` tree; `EpisodeService.searchEpisodes`/`DepartmentService.getPatients` bulk-fetch the roster once (per-episode `getPatient` re-fetched the full roster per id → N+1 over real MIS, roster loads never settled). Red runs triaged: `34326450227` (MisAuthServiceTest missing runPath → set it), `34327013868` (`Pattern.quote` in literal string assert → dropped), `34334338473` (DepartmentServiceTest stale per-id stubs → roster stubs), `34326450227/34327670581` (empty secrets → APP environment), `34329359643` (51 failures: roster N+1 + 300-deep MIS id probe → bounded 1..12/120s + U+FFFD literal destroyed by bulk script → restored (byte-audit clean) + candidates-gate skips for seed-bound legacy prosthetics specs + always-open list fallback fix + ad-login slot-9 demote restore + tablet patient-404 console gate). CI run `34350628013` all 6 jobs green (E2E 19m50s). Flake-check re-run `34353212923` all 6 jobs green (E2E 18m35s) — double-green confirmed, no flakes.

**2026-09-08: Prosthetics setup flow on eligibility candidates (issue #262, CLOSED — code-complete, CI green; close-summary recorded in issue comments)** — `PatientSearchPage` source switched to `prostheticsPatientApi.listCandidates` (debounce/abort/loading/empty/error preserved, no frontend eligibility filtering); `OrderSelectPage` renders MIS document badges from candidate response (`documentsUnknown` fallback). Backend `GET /api/prosthesis-manufacturing/patients/candidates` accepts optional `?query=` (name/ID contains filter). Fixtures: prosthetics seed patients `900001`/`900002` given `patientDepartmentID: 19` so setup-flow E2E stays eligible (parity-pinned, count 92). Tests: candidates query unit, migrated setup Vitest (candidates source, badges, unknown, api contract), authSecurityContract mocks updated, controller slice stubbed `isNull()` query. Red run triaged: `34233714093` (`ProstheticsPatientControllerTest` strict-stubs failure on zero-arg `getCandidates()` → stubbed `isNull()` + added forwarding test). CI run `34234391758` all 6 jobs green.

**2026-09-08: ICU roster via module filter (issue #261, CLOSED — code-complete, CI green; close-summary recorded in issue comments)** — same `?module=` contract as #260: `module=icu` narrows to department 19 via `PatientModuleFilter.ICU_DEPARTMENT_IDS` (no new endpoints); shared `PatientSearch` (sole consumer: `CreateCardPage`) moved to `patientApi.searchByModule('icu', …)` keeping 300ms debounce + `AbortController`, plus a new error state with retry (failures were swallowed silently); `CreateCardPage` untouched, `DepartmentDashboardPage` verify-only (local-DB scope, unfiltered name lookup). Fixture seed rows 1004/1005 remapped to 19 in `patients_92.json` (parity-pinned, count stays 92). Tests: icu filter unit + REST integration (same `AbstractIntegrationTest` context) + Vitest icu-source/debounce/abort/error/retry/empty/contract. Red runs triaged: `34226982261` (unknown-module probes still used `icu`, valid since this phase → switched to `surgery` in both suites). CI run `34227699533` all 6 jobs green.

**2026-09-08: Medication roster via module filter (issue #260, CLOSED — code-complete, CI green; close-summary recorded in issue comments)** — `GET /api/patients?module=medication` narrows the MIS search result to departments 19/37 via new `PatientModuleFilter` (unknown module → 400, null dept excluded; plain `?query=` contract untouched); `patientApi.searchByModule` + both prescription pages migrated, client `=== 1/2` filter deleted, surgery|rehab toggle splits 19|37 locally (`lib/medicationDepartments.ts`, persisted keys kept); skeleton loading, error `Alert` + retry, empty states. Fixture seed rows 1001/1002/1003 remapped 2→19/37/19 in `patients_92.json` (parity-pinned, count stays 92); rest of 1/2 rows are non-eligible others until #264. Tests: filter unit + REST integration on mixed stub data (same `AbstractIntegrationTest` context) + Vitest toggle/persist/empty/error/retry/skeleton/api-contract. Red runs triaged: `34213397228` (security-contract mocks + unknown-module body shape), `34214021232`→E2E red (3 specs opening 1002 without the rehab toggle → toggle selected). CI run `34216840110` all 6 jobs green. Post-close docs-run `34219434557` E2E red (2 `prescription-medication-cancel` context-menu timeouts on untouched detail grid, 3rd spec in file green) → flake-check re-run `34221999426` all 6 jobs green, flake confirmed.

**2026-09-08: Prosthetics eligibility candidates endpoint (issue #259, CLOSED — code-complete, CI green; close-summary recorded in issue comments)** — new `ProstheticsEligibilityService` (dept ∈ {19,27,37} ∧ EXISTS doc.template ∈ {120,121} → `{patient, orders[], documents[] with documentUrl, documentsUnknown}`); new `GET /api/prosthesis-manufacturing/patients/candidates` (same `@PreAuthorize`, old endpoints untouched); `departmentId` added to `ProstheticsPatientResponse` (source: `PatientDTO.departmentId`); `merge()` package-visible for eligibility reuse. Degradation: per-patient MIS docs error → `documentsUnknown` fail-open (debug log, no PII); null dept treated as ineligible (fixture rows lack it — NPE caught by integration test). N+1: no MIS batch → parallel fetch (deterministic order by MIS id) + 5-min TTL documents cache (failures uncached); dept/template/TTL are single-place assumptions for open blockers (b)/(c)/(f). Scope p.6 verified already-done (`MisOrderTemplateDataService` gone in #257 follow-up, zero consumers) → deleted `getServices`/`getPatientBookings`/`getPatientInfo` + parsers + 3 DTOs + 7 unit tests; runtime stubs/fixtures stay for #264 cutover (`stub_mapping.json` marked removed, `totalStubbed` 10→9). Tests: 13 unit (matrix + TTL hit/expiry via `MutableClock` + null-dept) + 2 controller-slice + 3 integration (real stub-HTTP chain via embedded-WireMock overrides, shared cached context). No frontend/E2E changes (Phase 9/14). Pre-flight: `mvn test-compile` clean, lint 0 errors. Red runs triaged: `34194191831` (two public ctors → `No default constructor found` in every prosthetics Spring test → `@Autowired` on primary), `34195285471`/`34198077717` (extra `@SpringBootTest` context exhausted CI Postgres `max_connections` → test reuses `TpLl02PdfIntegrationTest` context + per-test WireMock overrides removed in `AfterEach`). CI run `34200411900` all 6 jobs green.

**2026-09-07: MIS medicine catalog + allergy flow full removal (issue #258, CLOSED — code-complete, CI green; owner sign-off recorded in issue comments)** — `spiMedicineItemKindDetails` is the single source for the live medicine catalog; the allergy flow is removed entirely (owner signed off). Backend: extended `MedicineMisDTO` to 17 fields (legacy 4 + itemKind* + medicineCategory* + medicinePackage* with tolerant alias parsing for `itemKindATC`/`itemKindAtc` etc.); new `SPI_MEDICINE_PROCEDURE = "spiMedicineItemKindDetails"` in `WireMockMisServiceImpl` with real-mode branch (`useSpi() ? SPI_MEDICINE_PROCEDURE : "spzIBMedicineDictionary"`); tolerant `parseMedicineList` accepts `medicineList`/`medicines`/`items` arrays and a new `booleanOrNull` helper for tri-state flags. `MisService.searchMedicineCatalog` Javadoc rewritten — no more local cache. Deleted: `AllergyMisDTO`/`AllergyResponse`/`AllergyMapper`/`AllergyCache`/`AllergyCacheRepository`; `PrescriptionController` `GET /allergies` endpoint + `AllergyMapper` field (the `medicine-catalog` endpoint stays, rewired straight to `misService` without `MedicineCatalogService`); `MedicineCatalogService` + `MedicineCatalogCache` + `MedicineCatalogCacheRepository`; `MisEmbeddedWireMockConfig` no longer stubs `spzIBPatientAllergy`; fixture `patient_allergy.json` deleted. New `med/002-drop-allergy-and-medicine-cache.sql` (DROP TABLE `allergy_cache` + `medicine_catalog_cache` + both indexes; full rollback recreates both). Frontend: `AllergyItem` type + `AllergyWarning` component + `AllergyWarning.test.tsx` deleted; `prescriptionApi.getAllergies` deleted; `MedicineSearchInput` no longer takes `allergies` prop and no longer falls back to the hardcoded `fallbackCatalog` (errors surface as «Не вдалося завантажити каталог ліків з MIS» inline); `PrescriptionGrid`/`PrescriptionItemForm`/`PrescriptionDetailPage` no longer plumb `allergies`. Tests: `PrescriptionControllerTest` `getAllergies_returnsList` deleted; `PrescriptionRepositoryTest` `shouldCreateMedicineCatalogCache` + `shouldDetectHighRiskMedicine` + `shouldCreateAllergyCache` deleted; `MedicineCatalogServiceTest` deleted; `MisWireMockIntegrationTest` fixture + 4 `getPatientAllergies_*` tests deleted; frontend `endpoints.test.ts` `getAllergies` test deleted, `PrescriptionDetailPage.test.tsx` `getAllergies` mock + `mockGetAllergies` deleted, `MedicineSearchInput.test.tsx` allergy test replaced with a network-error surface test, `PrescriptionGrid`/`PrescriptionItemForm`/`PrescriptionSpreadsheet.test.tsx` `allergies` props removed. Pre-flight: `mvn compile` clean, 165 common + 141 medication-sheet unit-tests green (0 failures), frontend `npm run lint` 0 errors, `tsc --noEmit` clean, Vitest 770/770. CI run `34152000037` all 6 jobs green (E2E 362 passed / 14 skipped; the 1 allergy-guard failure fixed by deleting the obsolete spec; backend-startup timeout raised 60s→180s in `playwright.yml`). Follow-up: dangling `mappings/patient_allergy.json` stub deleted, `stub_mapping.json` allergy entry marked removed.

## Current Session

**2026-09-07: MIS medicine catalog + allergy flow full removal (issue #258)** — see top.

## Current Session

**2026-09-06: LDAP/AD authentication epic — milestone complete (issues #244–#251, all CLOSED, CI GREEN)** — directory authentication as a second source beside local BCrypt. Contract (#244, CLOSED): D1 source-by-row (LOCAL→BCrypt only, LDAP→bind only, unknown→bind+provision), D2 `auth_provider` column (sentinel hashes forbidden), D3 bind + `sAMAccountName` search with profile-only mapper (no AD-group auth), D4 first-login `GUEST` with zero rights, D5 attribute mapping, D6 isolated test double. Infra (#245, `2b29dc3` + 3 fixes): `spring-ldap-core` + `spring-security-ldap` (Boot BOM), conditional `LdapConfig` (pooled source, 5s timeouts, JVM-truststore TLS, `app.ldap.*` property layer over env), `LdapAuthService` closed-`Optional` seam, env contract `APP_LDAP_URLS/BASE/USERNAME/PASSWORD` (`APP_LPAD_PASSWORD` typo fixed per owner). Domain (#246, `0f2a286`): `UserRole.GUEST` (9 values), `auth_provider`, nullable `password_hash` via `core/007-ldap-auth-provider.sql` (guarded rollback — both branches validated on a DB copy against live-provisioned rows), race-safe provisioning, LOCAL collisions fail closed, JWT untouched. Frontend (#247, `0874f9d`+`8789eab`): single login form kept, GUEST empty-state `Alert` (repo Shadcn primitive; Shadcn MCP unavailable in this env), Vitest GUEST/storage cases. Cleanup (#248): verify-only — auth mocks never existed, MIS WireMock intact (15-row table in close comment). Tests (#249, `13058bc`): hermetic UnboundID 7.0.5 double (env-seeded by name, random service password), 7 double-integration + blackhole-timeout tests, all local-only (`-Dldap.local.tests=true`, SKIPPED in CI). E2E (#250, `07b5adc`): `ad-auth` helpers + `ad-login.spec` (10 scenarios, API setup/promote/demote, trace off, secrets nowhere), 30 passed / 0 failed locally; fixed session-flag gap (cookie alone bounces to `/login`) and sidebar-duplicate locators. Docs (#251): this AD Safety Directive, README Security, runbook env block. Deltas: backend +5 main / +7 test files, frontend +1 spec +1 helper +1 Vitest file, E2E +1 spec / +10 tests, Liquibase core 6→7. Green runs: `34022037197` (#246), `34024280768` (#247), `34027031777` (#249), `34031583873` (#250), `34044892718` (docs). Red runs triaged: JWT-tripwire default, Mockito strict-stubs (lazy mapper fallback), blank-value fail-fast determinism, UnboundID base-DN bootstrap, Spring 7 `HttpStatusCode`, `@Value` env-vs-property layering, Vitest missing login click, E2E session flag. Local LDAP suites green with the flag (unit 16+9, double-integration 7+1, E2E 30); corporate AD unreachable from CI by design. Kept intentionally: historical `MockMisServiceImpl` mentions in docs/comments (deleted impl, #194), `APP_TEST_USERROLE*` variables unread for authorization (D4).

**2026-09-05: Replacement removal — milestone complete (Epic #235, issues #236–#243, all CLOSED, CI GREEN)** — the «Створити замінювальний процес» flow is gone from code, tests, E2E and live docs; failed process is terminal (`IN_PROGRESS→FAILED→END` + `FAIL` audit). Frontend (`6a7c078`): deleted FailedScreen CTA/dialog/handler + `flowInstanceApi.replacement` + Vitest case, added `replacement-absent.spec.ts`. Backend (`90af16e`): deleted `POST /instances/{id}/replacement` + `FlowInstanceService.replacement()` (+ `REPLACEMENT` audit), controller 404 regression, narrowed `TpLl02RbacReplacementIntegrationTest` → `TpLl02RequireOwnerIntegrationTest`, linearized 3 E2E specs (failure spec renamed). Workflow (`38b1280`): verified FAILED terminal, added fail-from-non-`IN_PROGRESS` rejection test. Audit (`7786725`): zero `REPLACEMENT` in code, `FAIL` intact, audit mutation lists synced. Database: no-op verified (no replacement schema). Tests (`cc6049b`): QA-plan assert. Docs (`93834ed`): live docs linearized, counts synced (E2E 88 specs / 367 tests, Vitest 769). Green run `33977516316` all 6 jobs green. Kept intentionally: history entries mentioning replacement, historical phase/ADR/analysis docs, `tests/tmp-templates.json` (stale unreferenced API dump, no replacement refs).
...
**2026-08-26: Responsive/ShadCN Phase 5 — a11y/touch regression expansion (issue #179, CI GREEN ×2)** — tests `620f49c`, fix `cbd7e7a` (run `32967758601` all 6 jobs green) + empty-commit flake check `099d590` (run `32969919682` green). Red run `32967027144`: Vitest's CSS pipeline returns **empty string** for `index.css?raw` → the 3 reduced-motion contract assertions got `''` → fixed by reading the file from disk (`readFileSync(resolve(import.meta.dirname, '../../index.css'))`). **M1 scroll audit extended**: `no-horizontal-scroll.spec.ts` now covers nurse surfaces (`/icu/nurse`, `/icu/nurse/episode/a3333333…`, `/prescriptions/nurse` under `.auth/nurse.json` describe), HOD `/icu/doctor/department` (`.auth/hod.json`), prosthetist `/prosthetics/new/select-template` (patient+order draft via localStorage initScript; page redirects to select-order only when `draft.orderId` missing) — +5 routes ×3 viewports. **New E2E** `tests/specs/responsive/tablet-navigation.spec.ts` (3 serial tests @768 doctor): rail toggle round-trip (`aside[aria-label=Бічна панель]` ≤64→≥200→≤64px polled past the width transition, «Навігація» header label hidden↔visible, collapsed links resolve by name through the `title` attribute, hamburger stays mobile-only); breadcrumb containment on episode route («Пацієнти»/«День», `overflow-x-auto touch-pan-x` class contract, docWidth ≤ viewport+1); hamburger-free nav path (collapsed-rail icon → `/prescriptions/doctor`, deep-route breadcrumb «Пацієнти» back-link) — **all three assert zero console.error** via page-level collectors; rail state reset per test via `addInitScript(removeItem('app-sidebar-collapsed'))` (collapsed default comes from TABLET_RAIL_QUERY match + persisted toggle). tablet testMatch extended (now 7 specs). **M2 audit locked in Vitest** `src/test/responsive/focusMotion.test.ts`: full-opacity focus-ring contract on 8 ui primitives (`ring-ring` present, NO `ring-ring/NN` — repo grep confirms zero live opacity variants, only the #137 comment) + `ring-destructive`; reduced-motion coverage table (3 gates: check-pop/step-fade-in · fullscreen dialog morph · global primitive gate sheet/fade/scale/pulse/skeleton/stepper/dialog/select/dropdown/tooltip/popover) asserted token-by-token with ≥3 gates each ending `animation: none` (`animate-spin` loaders intentionally exempt — state communication); performance spot-check via `import.meta.glob('?raw')` asserts ZERO `addEventListener('resize')` in src (only col-resize cursor classes). Audit report appended to plan doc Phase 5.

**2026-08-25: Responsive/ShadCN Phase 4 — prosthetics & admin tablet pass (issue #178, CI GREEN ×2)** — feature `448cb2d`, fixes `90079af` (unrelated Vitest teardown flake) + `0acb1df`/`ba03eef` (E2E locator scoping); run `32944118175` all 6 jobs green + empty-commit flake check `32946134650` green. Red runs: `32938931764` (Vitest unhandled `window is not defined` — MedicineSearchInput onBlur setTimeout survived env teardown → timer tracked in a ref and cleared on unmount), `32939611904` (`getByPlaceholder`-style page-level `div.overflow-x-auto` strict violation — Base UI Tabs keeps inactive panels mounted AND shadcn Table nests its own `div.relative.w-full.overflow-x-auto.touch-pan-x` table-container inside the consumer wrapper → scroll the `.last()` th-filtered container; sticky column pins within it). M1 AdminPage: verified already compliant (stats `grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`, RBAC matrix sticky first column `sticky left-0 z-10 min-w-[260px] bg-card` inside `overflow-x-auto touch-pan-x`, TabsList scroller with `touch-pan-x` from ui/tabs) + added `min-w-[72px]` floor on matrix role-column headers (8 checkbox columns keep usable width at 768). **M2 ProcessLayout**: desktop branch collapses to an icon rail in the tablet band — nav `w-14 shrink-0 … p-2 lg:w-56 lg:p-3`, labels `hidden lg:inline`, «Процес #id» header `hidden … lg:block`, links `justify-center lg:justify-start` (CSS-first; <640px top-tab bar untouched; jsdom ProcessLayout.test.tsx unaffected — RTL finds hidden-class spans; no E2E clicks Огляд/Історія by name). **M3**: QualityGatePanel/SetupSteps/WizardScreen verification-only — decision buttons already `flex-col → sm:flex-row sm:flex-wrap` + `w-full sm:w-auto`, criteria rows `min-h-11`, SetupSteps `hidden md:inline`, wizard keeps flex-wrap toolbars + overflow-x-auto chips (no change). **Tests**: new `src/test/responsive/tabletPhase4.test.tsx` (5 ?raw contracts); new E2E `tests/specs/responsive/tablet-admin.spec.ts` (3 serial tests in responsive-tablet-chromium with `test.use({ storageState: '.auth/admin.json' })`: innermost matrix container scrolled to 400 with the «Операція» sticky column pinned ≤2px drift, toggle→dirty badge→save («Збережено змін: 1»)→revoke-toggle→save round-trip restoring shared RBAC state — note: after a saved grant the revoke toggle makes the draft dirty AGAIN, stats cards pairing into 2 columns via boundingBox y) and `tests/specs/responsive/tablet-prosthetics.spec.ts` (3 serial tests, PROSTHETIST storageState: dashboard overflow-free; process rail ≤80px wide with labels hidden but `a[href$="/history"]` navigation working; wizard stage visible + overflow-free; API-created instance via mobile-wizard-smoke's createInstance pattern completed in afterAll); `no-horizontal-scroll.spec.ts` gained `/prosthetics/new/select-order` (patient-draft seeded via localStorage initScript — OrderSelectPage redirects to select-patient only when `draft.patientId` missing); tablet testMatch extended. Plan doc Phase 4 outcome paragraph + sidebar component-map row updated.

**2026-08-25: Responsive/ShadCN Phase 3 — forms & dialogs adaptive layout at tablet widths (issue #177, CI GREEN ×2)** — code `c9e6ad5`, test fix `238df83` (run `32882482450` all 6 jobs green) + empty-commit flake check `794add8` (run `32885037081` green). First run `32879236343` failed only the new patient-state E2E: `getByPlaceholder('Година')` strict-mode violation — LabResultsPanel renders a second «Година» input in the same sidebar → fixed by scoping into the PatientStatePanel card via its «Свідомість» label; same run also flaked touch-targets («Нова карта» height 43.99998px < 44, subpixel rounding) → helper tolerance lowered to ≥43.5px. **M1 scale-form verification**: `ApacheIiForm` (sm:2/md:3/lg:4), `BradenForm` (sm:2/md:3), `RassSelector` trigger already had full tier ladders + `pointer-coarse:min-h-11` — no change; **`SofaForm` was the only gap** → its 13-field grid is now `grid grid-cols-1 gap-1.5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4`. **M2**: `VitalSignsForm` grid `sm:grid-cols-4` → `sm:grid-cols-3 md:grid-cols-4` + textarea `min-h-[60px] md:min-h-[72px]` (component currently has no production importer — test-only, kept); `PatientStatePanel` six select fields wrapped in `grid grid-cols-1 gap-1.5 md:grid-cols-2`, all inputs/triggers `pointer-coarse:min-h-11`, notes textarea `min-h-[2.5rem] md:min-h-[72px]`; `MedicalNotesPanel` fragment → outer `grid grid-cols-1 gap-2 lg:grid-cols-2` with the list wrapped in `flex flex-col` (editor | list side-by-side ≥1024px). **M3**: `ui/dialog.tsx` `DialogContent` base policy `sm:max-w-sm` → `sm:max-w-sm md:max-w-md` (384→448px ≥768), single source; narrow consumers keep intentional caps via cn()/tailwind-merge (`max-w-xs` on dose-execution popovers survives unconflicted — documented exception); `ui/sheet.tsx` unchanged — right-side patient sidebar sheet capped by consumer (`w-full p-0 sm:max-w-sm` = 384px @768). **Tests**: new `src/test/responsive/tabletForms.test.tsx` (9 ?raw contracts incl. dialog policy + sheet cap + max-w-xs retention), new `components/ui/dialog.test.tsx` (7 jsdom: closed-hidden, labelled open dialog, `sm:max-w-sm`/`md:max-w-md` classes, default no `data-fullscreen`, `mobileFullscreen`→`data-fullscreen="mobile"`, Escape onOpenChange, controlled open), new E2E `tests/specs/responsive/tablet-forms.spec.ts` (4 serial tests in `responsive-tablet-chromium`; testMatch extended) targeting **episode `a3333333` (Сидоренко) whose ONLY day `b3333333` stays OPEN suite-wide** (no spec signs it; doctor-project signoff specs touch a1111111/a2222222 only) so sidebar forms are editable by default — inline sidebar visible (`div.hidden.md\:block` hosting the Шкала combobox) + sheet trigger «Панель пацієнта» hidden at 768; SOFA form row-alignment via boundingBox y (items 1–2 same row, Тромбоцити wraps) + `pointer-coarse:min-h-11` class contract + calculate round-trip asserting the SOFA card flips to «Результат:»; patient-state Свідомість/Шкіра pair on one row with Набряки below; `/prescriptions/doctor` delete-dialog width asserted in (384, 450] then «Скасувати» (drawer flow: search «1002» → row «Відкрити» → drawer «Видалити»). Plan doc Phase 3 outcome paragraph + component-map rows updated (dialog ✅, sheet ✅ verified).
...
## Test isolation (no real MIS / no LDAP in CI)

Full Unit + Integration + Playwright E2E suites run in CI **without access to LDAP and without any interaction with the real MIS**. Local/Development and Production keep working with the real MIS and real LDAP. Issues #297–#302.

- **MIS sidecar-stub** (`tests/mis-stub/`, #297): a dependency-free Node HTTP server speaking the live envelope contract (`POST /token` form-grant + `POST /api/run {name, params[], installationId}` + served `/doc/*` URLs + `GET /health`). Data is hand-crafted and PII-free (`fixtures.json`); contract pinned by `node --test` (`stub-contract-test.cjs`, invisible to Playwright by filename) and by `MisStubContractTest` (fixtures through the `MisServiceImpl` parsers, #298). The stub lives **only** in `tests/` — no fakes in `backend/*/src/main`, never started in prod.
- **LDAP is disabled in CI**: `APP_LDAP_ENABLED: "false"` in the `backend-test`, `backend-integration` and `e2e-test` job envs (#299) — directory beans are not created, logins take the BCrypt path. Code default of `app.ldap.enabled` is **`false`** (`application.yml:90-91`); the flag is pinned explicitly anyway as belt-and-braces (during #302 verification a foreign uncommitted WIP was found flipping it to `true` — the explicit CI value neutralized it). Local sets `APP_LDAP_ENABLED=true` explicitly.
- **Test authentication**: `UserSeedService` LOCAL accounts (`APP_TEST_USERNAME1..9` / `APP_TEST_PASSWORD1..9`, `environment: APP`) + the real `JwtTokenProvider` — no separate JWT issuer or auth mocks. `LdapDisabledInTestTest` proves the contract (ungated, runs in CI); LDAP suites stay gated (`ldap.local.tests` / `hasAdEnv`) and report SKIPPED in CI; `ad-login.spec.ts` keeps its skip-gate.
- **Tripwires** (#298): `NoRealMisInTestsTripwireTest` (no `APP_MIS_API_*` names, no external HTTP hosts, `ldap://` only in local-only shapes), `MisWriteBanTripwireTest` (no `sendPdf` in `src/main`).
- **Secrets rule**: `APP_MIS_API_*` / `APP_LDAP_*` values live only in local/prod environments. CI maps **zero** MIS/LDAP secrets (E2E backend points at `http://127.0.0.1:9099` with `e2e-stub` credentials).

| Component | Local/Development | CI/Test | Production |
|---|---|---|---|
| LDAP | **enabled** (`APP_LDAP_ENABLED=true`) | **disabled** (`APP_LDAP_ENABLED=false` in 3 jobs) | **enabled** (env, explicit) |
| Real MIS | **enabled** (real `APP_MIS_API_*`) | **not used by tests** (stub `127.0.0.1:9099`) | **enabled** (env, explicit) |
| Test doubles MIS | not used | **used** (`tests/mis-stub/`) | not used |
| Test authentication | not used | **used** (LOCAL seeds + real JWT) | not used |
| Real LDAP authentication | **yes** | **no** | **yes** |

---

## Current Session

**2026-09-08: Remove WireMock and obsolete runtime mock data (issue #263, CLOSED — code-complete, CI green; close-summary recorded in issue comments)** — Phase 10 of the MIS real-API epic. Deleted `mis-wiremock/` fixture tree (29 files), `MisEmbeddedWireMockConfig`, `MockMedicalInformationSystemController`, `MisErrorSimulator`, `MisDictionaries`, `MisModeGuard`, `MisAuthService`; stripped `WireMockMisServiceImpl` (removed `useSpi()` toggle and all real-mode branches, keeping only the WireMock/spzIB-shaped contracts), `MisApiClient` (removed dual-mode, Bearer auth, and `MisApiProperties`/`MisAuthService` coupling — single-mode POST to `app.mis.api.base-url` + `run-path`), `MisApiProperties` (removed `mode`, `isRealMode`, `assertRealModeReady`), `MisService` (removed `setErrorMode` default method). Properties: removed `wiremock-enabled`, `embedded-wiremock-enabled`, `wiremock-port`, `wiremock-url`, `mode` from `application.yml`; kept `app.mis.api.*` knobs for future real-MIS cutover. POM: removed `wiremock-standalone` dependency. Tests: deleted `WireMockMisServiceImplTest`, `SpiPatientServiceTest`, `SpiDocumentServiceTest`, `MisWireMockIntegrationTest`, `MisParityTest`, `MisAuthServiceTest`, `MisMutualExclusionTest`, `ProstheticsEligibilityIntegrationTest`; updated remaining integration tests (`AbstractIntegrationTest`, `LdapTimeoutIntegrationTest`, `ProdSwaggerGatingIntegrationTest`, `SeedDataPasswordIdempotencyIntegrationTest`, `BrakIntegrationTest`, `CrossFeatureRegressionIntegrationTest`) to drop obsolete properties; rewrote `MisApiClientTest` for single-mode; updated `ModuleBoundaryTest` exclusions. E2E: deleted `api-error-mode-chromium` project + `mis-error-scenarios.spec.ts`; removed `/api/mis/error-mode` reset calls from 8 specs + 1 comment. Pre-flight: `mvn compile` clean, `npm run lint` 0 errors, `tsc --noEmit` clean. CI run `34250545502` all 6 jobs green (Backend Tests / Frontend Tests / Backend Integration Tests / Code Quality / E2E Tests / Build Artifacts).
...
**The ICU Chart module is a READ-ONLY client of MIS — with no exceptions.** Only data retrieval from MIS is permitted. Generated PDFs stay local (download/print in-module, Phase 16, #269) and are never transferred to MIS.
...
| Config | `app.ldap.enabled` (code default `false` in `application.yml:90-91`; CI pins `APP_LDAP_ENABLED=false` explicitly in 3 jobs as belt-and-braces, local sets `=true` explicitly); `APP_LDAP_URLS` / `APP_LDAP_BASE` / `APP_LDAP_USERNAME` / `APP_LDAP_PASSWORD` via environment only, no defaults, never mapped in CI; TLS via JVM truststore, never TrustAll |
...
backend/   (Spring Boot 4.1.0 + Java 25 + Maven, multi-module; dependency direction: common ← feature modules ← app)
  ...
  medication-sheet/         ← medication sheet feature (93 main sources): `com.superhumans.medicationsheet.*` (entity/dto/repository/service/controller/mapper/config); depends on common
  prosthesis-manufacturing/ ← prosthetics manufacturing feature (89 main sources): `com.superhumans.prosthesismanufacturing.*` (entity/dto/repository/service/controller/mapper/config); depends on common
...
| `e2e-test` | Playwright (98 spec files, chromium, 40-min timeout; `needs: backend-test, frontend-test`) | `playwright-report`, `playwright-test-results` |
...
| `npm t` or `npx vitest run` | Run Vitest tests (930 tests across 100 files) |

### Playwright (`cd tests`)
| Command | Action |
|---|---|
| `npx playwright test` | Run all E2E tests (98 spec files) |
| `npx playwright test --list` | List tests without running |
| `npx playwright show-report` | View HTML report |

## Testing

- **Backend**: 390 main sources / 185 test files across the multi-module reactor (common 125/29, icu-chart 83/76, medication-sheet 93/30, prosthesis-manufacturing 89/49, app 0/1 — the app test is the ArchUnit `ModuleBoundaryTest`). JaCoCo 60% instruction / 50% branch minimum. Checkstyle Google checks.
...
Application users are NOT in SQL: `UserSeedService` provisions 9 `LOCAL` accounts at boot (BCrypt-hashed at seed time, existing logins never overwritten). Logins/passwords come **only** from environment (`APP_TEST_USERNAME1..9` / `APP_TEST_PASSWORD1..9`, CI: secrets, job-level `environment: APP`); profile metadata — from `APP_TEST_USERFULLNAME[n]` / `APP_TEST_EMAIL[n]` / `APP_TEST_PHONE[n]` / `APP_TEST_PROFESSION[n]` / `APP_TEST_USERROLE[n]`. E2E resolves them via `testUser(slot)` (`tests/helpers/test-users.ts`). AI agents reference env **names** only — never values. Local runs set `APP_LDAP_ENABLED=true` explicitly (code default of `app.ldap.enabled` is `false`; CI pins it `false` explicitly as belt-and-braces — see Test isolation).

| Слот | ПІБ | Посада | Роль | Логін / пароль (env) | Телефон |
|---|---|---|---|---|---|
| 1 | Олександр Мельник | Лікар-хірург | DOCTOR | `APP_TEST_USERNAME1` / `APP_TEST_PASSWORD1` | +380501111111 |
| 2 | Наталія Бойко | Лікар-хірург | DOCTOR | `APP_TEST_USERNAME2` / `APP_TEST_PASSWORD2` | +380502222222 |
| 3 | Олена Ткаченко | Медична сестра стаціонару | NURSE | `APP_TEST_USERNAME3` / `APP_TEST_PASSWORD3` | +380503333333 |
| 4 | Марія Кравчук | Медична сестра стаціонару | NURSE | `APP_TEST_USERNAME4` / `APP_TEST_PASSWORD4` | +380504444444 |
| 5 | Василь Гончарук | Завідувач | HEAD_OF_DEPARTMENT | `APP_TEST_USERNAME5` / `APP_TEST_PASSWORD5` | +380505555555 |
| 6 | Адмін Системи | Адміністратор | ADMINISTRATOR | `APP_TEST_USERNAME6` / `APP_TEST_PASSWORD6` | +380506666666 |
| 7 | Олег Романюк | Протезист | PROSTHETIST | `APP_TEST_USERNAME7` / `APP_TEST_PASSWORD7` | +380507777777 |
| 8 | Ірина Шевчук | Протезист | PROSTHETIST | `APP_TEST_USERNAME8` / `APP_TEST_PASSWORD8` | +380508888888 |
| 9 | Тарас Мельник | Адміністратор протезування | PROSTHETICS_ADMINISTRATOR | `APP_TEST_USERNAME9` / `APP_TEST_PASSWORD9` | +380509999999 |
...
| `Permission` | — | code(PK), label, description, category | Dictionary of the RBAC catalog (28 codes) |
| `RolePermission` | — | role(PK, UserRole), permissionCode(PK→Permission) | Default-deny grants; presence = granted |
| `GeneratedPdf` | BaseEntity | clinicalDay(M→1), fileName, fileVersion, generatedAt, generatedBy, checksum, fileData(byte[]) | Transfer columns + `TransferStatus` removed in #269 (PDFs stay local — see Liquibase `icu/007-drop-pdf-transfer.sql`) |
...
| GET | `/api/clinical-days/{id}/pdf/file` | Yes | Serve stored PDF bytes (`application/pdf`); the transfer-status endpoint was removed in #269 (PDFs stay local) |
...
| GET | `/api/prosthesis-manufacturing/production` | Yes (`PROSTHETICS_PRODUCTION_VIEW`; assignee forced to self without `VIEW_ALL`) | Production work items, paged |
| GET | `/api/prosthesis-manufacturing/production/summary` | Yes (`PROSTHETICS_PRODUCTION_VIEW`) | 9 KPI cards |
| GET | `/api/prosthesis-manufacturing/production/attention` | Yes (`PROSTHETICS_PRODUCTION_VIEW`) | Attention queue, severest first |
| GET | `/api/prosthesis-manufacturing/production/team` | Yes (`PROSTHETICS_PRODUCTION_VIEW_ALL`) | Workload per prosthetist |
| GET | `/api/prosthesis-manufacturing/production/{id}` | Yes (`PROSTHETICS_PRODUCTION_VIEW`; masked without `PATIENT_VIEW`) | Work-item detail |
| GET | `/api/prosthesis-manufacturing/production/settings/normative` | Yes (`PROSTHETICS_PRODUCTION_VIEW_ALL`) | Read overdue/stale normatives |
| PUT | `/api/prosthesis-manufacturing/production/settings/normative` | Yes (`PROSTHETICS_PRODUCTION_VIEW_ALL`) | Update normatives (audited) |
...
| `/prosthetics/production` | `ProductionPage` | PROSTHETIST, PROSTHETICS_ADMINISTRATOR (or `MODULE_PROSTHETICS_ACCESS`) + `PROSTHETICS_PRODUCTION_VIEW` |
...
| `prosthetics/` | `ProstheticsDashboard.tsx`, `DashboardPage.tsx`, `ProductionPage.tsx`; `setup/` — `PatientSearchPage`, `OrderSelectPage`, `OrderReviewPage`, `TemplateSelectPage` + steps (`OrderStep`, `PatientStep`, `ReviewStep`, `TemplateStep`); `process/` — `ProcessDetail`, `ProcessHistoryPage`, `ProcessLayout`, `ProcessOverview`, `WizardScreen`, `DoneScreen`, `FailedScreen`, `MeasurementForms` |
...
| `components/prosthetics/` | `StatusBadge`, `SetupSteps`, `ProcessStat`, `ProductionWorkItemDrawer`, `ProductionNormativeSettings`, `ProductionTrendChart` |
| `components/navigation/` | `AppSidebar.tsx`, `Breadcrumbs.tsx` |
| `components/ui/` | shadcn-style Base UI primitives: `button`, `input`, `card`, `dialog`, `table`, `select`, `tabs`, `switch`, `checkbox`, `radio-group`, `dropdown-menu`, `popover`, `tooltip`, `progress`, `skeleton`, `sonner`, `chart` (Recharts port for the production trend), … |

### API Client (`frontend/src/api/`)
- **`client.ts`**: Axios instance → `http://localhost:8085/api`, JWT interceptor
- **Per-feature modules** (no barrel): `platform.ts` (auth, patient, user, settings, audit, admin), `icu.ts` (episode, clinicalDay, hourlyRecord, medicalOrder, orderExecution, medicalNote, clinicalScale, fluidBalance, pdf, patientState, ventilation, labResult, department), `medication.ts` (prescription, vitalSign); prosthetics APIs in `prosthetics.ts` (isolated, incl. `productionApi`: list/team/attention/summary/detail/normative). Shared DTO types live in `types/core.ts`, ICU types in `types/icu.ts`, medication types in `types/medication.ts`; production DTOs live in `src/prosthetics/types.ts`, the trend `TrendPoint` in `lib/productionTrend.ts`.
...
### Response DTOs
`LoginResponse`, `EpisodeResponse`, `ClinicalDayResponse`, `HourlyRecordResponse`, `MedicalOrderResponse`, `OrderExecutionResponse`, `MedicalNoteResponse`, `ScaleResultResponse`, `FluidBalanceResponse`, `SignResponse`, `PdfResponse`, `UserResponse`, `AuditLogResponse`, `ErrorResponse` (14 total). Prosthetics production DTOs (`ProductionWorkItemDto`, `ProductionSummaryDto`, `ProductionDetailDto`, `ProductionTeamRowDto`, `ProductionNormativeDto`) live in the prosthesis-manufacturing module.
...
| `PermissionCatalog` | RBAC catalog: 28 permission codes across 8 categories, role defaults for seeding |
...
### Prosthetics manufacturing (`prosthesis-manufacturing`, 12)
| Service | Responsibility |
|---|---|
...
| `ProductionReadService` | Production read-model: list/summary/attention/team/detail with batch aggregates + flags |
| `ProductionNormativeService` | Overdue/stale normatives over `SystemSettings` (defaults K=1.5, 7 days) |
...
| §— | Liquibase schema management | `ddl-auto: none`, schema per DB via `db/changelog/db.changelog-master-{core,icu,med,prosth}.yaml` (26 SQL files: core 8, icu 7, med 2, prosth 9); seed data via `SeedDataInitializer` (`data-{core,icu,med,prosth}.sql`, gated by `app.seed-data.enabled`) |
...
- **Notifications**: transient action feedback uses sonner `toast.*` at the page-handler layer (`Toaster` mounted globally in `App.tsx`; E2E asserts via `[data-sonner-toast][data-type]`); inline `Alert` is reserved for page-load failures. Prescription detail page follows the prosthetics (`WizardScreen`) convention.
...
- **DB**: `ddl-auto: none` — schema per DB managed by the Liquibase changelogs in `db/changelog/{core,icu,med,prosth}/` (master yamls + 26 SQL files); never write manual DDL
