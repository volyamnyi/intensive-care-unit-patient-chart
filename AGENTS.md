...
**2026-09-07: Order document via MIS — local PDF generator removed (issue #257 follow-up, in_progress)** — `spiDocumentProsthesCheck.documentUrl` is the single source for the prosthetics order document. ICU Chart no longer generates a recipe PDF locally. Backend: deleted `recipe_pdf_data`/`recipe_pdf_generated_at` columns on `prosthetics_orders` (Liquibase `prosth/009-drop-order-recipe-pdf.sql`, full rollback), deleted `ProstheticsOrderService.getRecipePdf`/`generateRecipe`/`getPdfInfo` + `PdfDocument` record, deleted `ProstheticsOrderController` `GET /orders/{id}/document` endpoint, deleted `ProstheticsPdfService.generateOrderRecipe(...)` (both overloads) and all 700+ lines of helpers (page-1 + page-2, fittings, materials, MIS fallback). `MisOrderTemplateDataService`/`MisOrderTemplateData` (used only by the old generator) deleted. New `GET /orders/{id}/document-url` resolves the MIS-hosted URL via `misService.getPatientDocuments(patientId)` and returns `{documentId, documentTemplateName, documentUrl}` (404 if no `documentUrl` on the matched document — guards against empty WireMock fixtures). `ProstheticsPdfResponse` DTO deleted (only consumed by the removed methods); `ProstheticsOrderResponse.hasRecipePdf` removed; `ProstheticsOrderMapper` no longer maps it. `FlowInstanceService` keeps `pdfService` for `generateFinalReport`/`generateFailureReport` (instance-issued reports, not the order document). Frontend: `prostheticsOrderApi.getDocument` → `getDocumentUrl` (no `Blob`/`createObjectURL`); `OrderReviewPage` renders the MIS URL directly in an `<iframe src={documentUrl}>` with an external-link button to «Відкрити замовлення на протез у MIS» (no local PDF, no object-URL leak). `ProstheticsPdfServiceTest`: 3 order-recipe tests deleted; constructor no longer takes `ObjectMapper`. `ProstheticsOrderServiceTest` shrunk from 10 to 7 cases (recipe generate/lazy/skip/info deleted); `pdfService`/`misTemplateDataService`/`auditService` mocks removed. `ProsthesisRepositoryTest` and `InstanceMapperTest` no longer set/assert `recipePdfData`/`hasRecipePdf`. `prostheticsApi.test.ts` + `OrderReviewPage.test.tsx` updated for the new `getDocumentUrl` contract. Pre-flight: backend `mvn compile` + 13 unit tests green, frontend `npm run lint` 0 errors, `tsc --noEmit` clean, Vitest 40/40 (prostheticsApi + OrderReviewPage). No CI run yet.

## Current Session

**2026-09-07: Order document via MIS — local PDF generator removed (issue #257 follow-up)** — see top.
