...
**2026-08-25: Responsive/ShadCN Phase 4 — prosthetics & admin tablet pass (issue #178)** — M1–M3 complete, pre-flight green. **M1 AdminPage**: verified already compliant (stats `grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`, RBAC matrix sticky first column `sticky left-0 z-10 min-w-[260px] bg-card` inside `overflow-x-auto touch-pan-x`, TabsList scroller with `touch-pan-x` from ui/tabs) + added `min-w-[72px]` floor on matrix role-column headers (8 checkbox columns keep usable width at 768). **M2 ProcessLayout**: desktop branch collapses to an icon rail in the tablet band — nav `w-14 shrink-0 … p-2 lg:w-56 lg:p-3`, labels `hidden lg:inline`, «Процес #id» header `hidden … lg:block`, links `justify-center lg:justify-start` (CSS-first; <640px top-tab bar untouched; jsdom ProcessLayout.test.tsx unaffected — RTL finds hidden-class spans). **M3**: QualityGatePanel/SetupSteps/WizardScreen verification-only — decision buttons already `flex-col → sm:flex-row sm:flex-wrap` + `w-full sm:w-auto`, criteria rows `min-h-11`, SetupSteps `hidden md:inline`, wizard keeps flex-wrap toolbars + overflow-x-auto chips (no change). **Tests**: new `src/test/responsive/tabletPhase4.test.tsx` (5 ?raw contracts); new E2E `tests/specs/responsive/tablet-admin.spec.ts` (3 serial tests in responsive-tablet-chromium with `test.use({ storageState: '.auth/admin.json' })`: sticky column pinned under forced `scrollLeft=400`, toggle→dirty badge→save («Збережено змін: 1»)→revoke-toggle→save round-trip restoring shared RBAC state — note: after a saved grant the revoke toggle makes the draft dirty AGAIN, stats cards pairing into 2 columns via boundingBox y) and `tests/specs/responsive/tablet-prosthetics.spec.ts` (3 serial tests, PROSTHETIST storageState: dashboard overflow-free; process rail ≤80px wide with labels hidden but `a[href$="/history"]` navigation working; wizard stage visible + overflow-free; API-created instance via mobile-wizard-smoke's createInstance pattern completed in afterAll); `no-horizontal-scroll.spec.ts` gained `/prosthetics/new/select-order` (patient-draft seeded via localStorage initScript — page redirects to select-patient only when `draft.patientId` missing); tablet testMatch extended.
