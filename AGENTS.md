...
- **LDAP is disabled in CI**: `APP_LDAP_ENABLED: "false"` in the `backend-test`, `backend-integration` and `e2e-test` job envs (#299) — directory beans are not created, logins take the BCrypt path. Code default of `app.ldap.enabled` is **`true`** (`application.yml:90-91`); the old docs claiming «default `false`» were wrong and are fixed below — CI disables it explicitly, local sets `APP_LDAP_ENABLED=true` explicitly.
...
| Config | `app.ldap.enabled` (code default `true` in `application.yml:90-91` — CI sets `APP_LDAP_ENABLED=false` explicitly in 3 jobs, local sets `=true` explicitly); `APP_LDAP_URLS` / `APP_LDAP_BASE` / `APP_LDAP_USERNAME` / `APP_LDAP_PASSWORD` via environment only, no defaults, never mapped in CI; TLS via JVM truststore, never TrustAll |
