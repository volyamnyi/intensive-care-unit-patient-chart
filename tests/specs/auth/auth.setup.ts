...
const USERS = [
  ...
  { login: 'prosthetist1', password: 'prosthetist123', file: 'prosthetist.json' },
  { login: 'prosthetics_admin1', password: 'prosthetist123', file: 'prosthetadmin.json' },
];
...
for (const user of USERS) {
  setup(`authenticate as ${user.login}`, async ({ page }) => {
    ...
    await page.waitForURL(/\/(doctor|nurse|admin|select|prescriptions)/);
