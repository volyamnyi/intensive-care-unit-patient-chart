...
export function ProcessExecution({ process, template, onComplete, onQualityCheck, onPause, onMainMenu, stageIndex, stepIndex, onUpdateProgress }: Props) {
  ...
  const handleNext = () => {
    // Check if next step is in same stage
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          {/* Dynamic controls */}
          <div className="bg-white border border-gray-200 rounded p-5">
            <h3 className="text-gray-700 text-sm mb-4">Дані для заповнення</h3>
            <div className="space-y-4">
              {currentStep.controls.map((control, ci) => (
                <div key={ci}>
                  <label className="block text-sm text-gray-700 mb-1">
                    {control.label}
                    {control.required && <span className="text-gray-500 ml-1">*</span>}
                  </label>
                  {control.type === "checkbox" && (
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" className="rounded border-gray-400" />
                      <span className="text-sm text-gray-600">Підтверджую виконання</span>
                    </label>
                  )}
                  {control.type === "text" && (
                    <input type="text" className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-500" placeholder={`Введіть ${control.label.toLowerCase()}...`} />
                  )}
                  {control.type === "number" && (
                    <input type="number" className="w-48 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-500" placeholder="0" />
                  )}
                  {control.type === "dropdown" && (
                    <select className="w-64 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none">
                    ...
                    <div className="flex gap-4">
                      {control.options?.map(opt => (
                        <label key={opt} className="flex items-center gap-2 text-sm cursor-pointer">
                          <input type="radio" name={`radio-${ci}`} /> {opt}
                        </label>
                      ))}
                    </div>
                  )}
                  {control.type === "textarea" && (
                    <textarea className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-500 h-20 resize-none" placeholder="Введіть..." />
                  )}
                  {control.type === "date" && (
                  ...
                  {control.type === "file" && (
                    <div className="border-2 border-dashed border-gray-300 rounded p-4 text-center">
                      <div className="text-gray-400 text-sm">Перетягніть файл або <span className="text-gray-600 underline cursor-pointer">оберіть файл</span></div>
                    </div>
                  )}
                  {control.type === "image" && (
                    <div className="border-2 border-dashed border-gray-300 rounded p-4 text-center">
                      <div className="text-gray-400 text-sm">Перетягніть зображення або <span className="text-gray-600 underline cursor-pointer">завантажте фото</span></div>
                    </div>
                  )}
                  {control.type === "signature" && (
                    <div className="border border-gray-300 rounded h-24 bg-gray-50 flex items-center justify-center">
                      <span className="text-gray-400 text-sm">Область підпису — клікніть для підписання</span>
                    </div>
                  )}
                </div>
              ))}
