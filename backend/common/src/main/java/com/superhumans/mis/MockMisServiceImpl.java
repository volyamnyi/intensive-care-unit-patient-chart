...
public class MockMisServiceImpl implements MisService {
    ...
    public List<ServiceMisDTO> getServices() {
        checkErrors();
        auditService.logAction("MIS", null, "GET_SERVICES", getCurrentUserId());
        return List.of(
                ServiceMisDTO.builder().serviceId(101L).serviceName("Протез передпліччя з тяговим керуванням")
                        .serviceCode("06 18 09").serviceDuration(120).servicePrice(150000.0).build(),
                ServiceMisDTO.builder().serviceId(102L).serviceName("Протез плеча з тяговим керуванням")
                        .serviceCode("06 18 12").serviceDuration(180).servicePrice(180000.0).build(),
                ServiceMisDTO.builder().serviceId(103L).serviceName("Протез кисті косметичний")
                        .serviceCode("06 18 03").serviceDuration(90).servicePrice(60000.0).build(),
                ServiceMisDTO.builder().serviceId(104L).serviceName("Протез після вичленення в променезап'ястковому суглобі")
                        .serviceCode("06 18 06").serviceDuration(150).servicePrice(120000.0).build());
    }

    @Override
    public List<BookingMisDTO> getPatientBookings(Long patientId) {
        checkErrors();
        auditService.logAction("MIS", null, "GET_PATIENT_BOOKINGS", getCurrentUserId());
        if (patientId == null) {
            return List.of();
        }
        return List.of(
                BookingMisDTO.builder().bookingId(2001L).bookingName("Силіконовий чохол")
                        .bookingDate(LocalDateTime.now()).patientId(patientId)
                        .serviceId(101L).serviceCode("SC-211")
                        .bookingServicePriceValue(15000.0).bookingQuantity(1)
                        .bookingStatusCode("COMPLETED").bookingPaymentStatusCode("PAY").build(),
                BookingMisDTO.builder().bookingId(2002L).bookingName("Поліпропілен")
                        .bookingDate(LocalDateTime.now()).patientId(patientId)
                        .serviceId(101L).serviceCode("PP-04")
                        .bookingServicePriceValue(800.0).bookingQuantity(1)
                        .bookingStatusCode("COMPLETED").bookingPaymentStatusCode("PAY").build());
    }
    ...
    public List<DocumentMisDTO> getPatientDocuments(Long patientId) {
        checkErrors();
        auditService.logAction("MIS", null, "GET_PATIENT_DOCUMENTS", getCurrentUserId());
        if (patientId == null) {
            return List.of();
        }
        return List.of(
                DocumentMisDTO.builder().documentId(3001L)
                        .documentName("Направлення на протезування")
                        .documentCreationDate(LocalDateTime.now().minusDays(5))
                        .documentKindCode("REF").documentKindName("Направлення")
                        .documentApproveStatusCode("APR").documentApproveStatusName("Затверджено")
                        .documentTemplateName("Направлення на протезування").build());
    }

    @Override
    public Optional<PatientInfoMisDTO> getPatientInfo(Long patientId) {
        checkErrors();
        auditService.logAction("MIS", null, "GET_PATIENT_INFO", getCurrentUserId());
        PatientDTO patient = patients.get(patientId);
        if (patient == null) {
            return Optional.empty();
        }
        return Optional.of(PatientInfoMisDTO.builder()
                .patientId(patient.getId())
                ...
                .patientSexCode(patient.getSexCode())
                .accountValue(0.0)
                .patientBookingSum(15800.0)
                .patientBookingAct(getPatientBookings(patientId))
                .patientDebtSum(0.0)
                .companyName("Медичний центр БО «БФ «СУПЕРЛЮДИ»")
                .companyEDRPOU("44803597")
                .build());
    }
}
