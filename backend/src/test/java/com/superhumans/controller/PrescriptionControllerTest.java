...
class PrescriptionControllerTest {
    ...
    void createPrescription_shouldReturnPrescription() throws Exception {
        ...
        Prescription prescription = Prescription.builder()
                .id(1L).medication("Dopamine").dose("200 mg")
    ...
    void executePrescription_stoppedPrescription_shouldReturnBadRequest() throws Exception {
    ...
    @Test
    @WithMockUser(username = "nurse1", roles = "NURSE")
    void executePrescription_expiredPrescription_shouldReturnBadRequest() throws Exception {
        when(prescriptionService.executePrescription(eq(2L), eq(5L), eq(10), eq(180), eq("nurse1")))
                .thenThrow(new BadRequestException("Cannot execute expired prescription"));

        Map<String, Object> body = Map.of("dayId", 5, "hour", 10, "actualVolume", 180);

        mockMvc.perform(post("/api/prescriptions/2/execute")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isBadRequest());
    }
    ...
    void executePrescription_shouldReturnFluidIntake() throws Exception {
        FluidIntake intake = FluidIntake.builder()
                .id(1L).hour(10).volumeActual(180)
                .medicationName("Dopamine")
                .status(ExecutionStatus.DONE)
                .build();
        when(prescriptionService.executePrescription(eq(1L), eq(5L), eq(10), eq(180), eq("nurse1")))
