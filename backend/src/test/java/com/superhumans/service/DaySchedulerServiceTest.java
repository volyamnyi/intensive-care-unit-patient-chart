...
class DaySchedulerServiceTest {
    ...
    @Mock private IcuDayService icuDayService;
    @Mock private UserRepository userRepository;
    @Mock private EmailService emailService;
    @InjectMocks private DaySchedulerService daySchedulerService;
    ...
    void autoCloseDays_shouldClosePreviousDay() {
        ...
        IcuDay yesterday = IcuDay.builder()
                .id(1L).dayNumber(1).date(LocalDate.now().minusDays(1)).status(DayStatus.ACTIVE).build();
        IcuDay today = IcuDay.builder()
                .id(2L).dayNumber(2).date(LocalDate.now()).status(DayStatus.ACTIVE).build();
    ...
    void checkEscalations_shouldSendEmailsToHeads() {
        ...
        IcuDay unsignedDay = IcuDay.builder()
                .id(1L).dayNumber(2).date(LocalDate.now().minusDays(1)).status(DayStatus.ACTIVE)
                .escalationSent(false).icuCard(card).build();

        when(icuDayService.getUnsignedDaysBeforeDate(any())).thenReturn(List.of(unsignedDay));

        User head = User.builder().id(1L).login("head1").email("head@hospital.ua")
                .role(UserRole.HEAD_OF_DEPARTMENT).build();
        ...
        assertTrue(unsignedDay.getEscalationSent().booleanValue());
    }

    @Test
    void checkEscalations_shouldSkipWhenAlreadySent() {
        IcuDay day = IcuDay.builder().id(1L).dayNumber(1).date(LocalDate.now().minusDays(1))
                .status(DayStatus.ACTIVE).escalationSent(true).build();

        when(icuDayService.getUnsignedDaysBeforeDate(any())).thenReturn(List.of(day));

        daySchedulerService.checkEscalations();
        verify(emailService, never()).sendEscalation(anyString(), anyString(), anyInt(), any());
    }

    @Test
    void checkEscalations_shouldSkipWhenNoHeads() {
        IcuCard card = IcuCard.builder().id(1L).patientName("Patient").build();
        IcuDay day = IcuDay.builder().id(1L).dayNumber(1).date(LocalDate.now().minusDays(1))
                .status(DayStatus.ACTIVE).escalationSent(false).icuCard(card).build();
